...
static bool parse_args(int argc, char **argv, std::size_t &bkgd_dim, std::string &sampler_path);
...
int main(int argc, char **argv)
{
    ...
    mesh::active active_piece(tex_atlas, game.active_piece(), game.level(), game.state(), game.frame_num());
...
{
public:
    window_exception(const std::string &msg) : msg(msg) {}
...
static GLFWwindow *init(float &proj_xx, float &proj_yy)
{
    ...
    GLFWwindow *window = glfwCreateWindow(mode->width, mode->height, "Tetris the Grand Master I (Palmer Edition)", primary_monitor, nullptr);
    ...
    {
        glfwTerminate();
        throw window_exception("Failed to initialized OpenGL throught GLEW");
    }
...
static bool parse_args(int argc, char **argv, std::size_t &transparency, std::string &sampler_path)
{
    if (argc == 1)
    {
        transparency = 10;
    ...
    else if (argc == 2)
    {
        if (strlen(argv[1]) == 1)
        {
            transparency = std::stoul(argv[1]);
            sampler_path = DEFAULT_SAMPLER_PATH;
            return true;
        }
        else if (strcmp(argv[1], "--help") == 0 or strcmp(argv[1], "-h") == 0)
        {
            std::cout << "Usage: " << argv[0] << " [transparency (0 to 9)] [sampler_path]" << std::endl;
            return false;
        }
        else
        {
    ...
    {
        throw std::invalid_argument("Arguments to the program invalid. Use --help or -h for help");
    }
}
